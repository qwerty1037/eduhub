  ...
  GridView twoColumnProblemList(ProblemListController controller) {
    return GridView.count(
        ...
        children: controller.currentPageProblems.map((element) {
          return Button(
            onPressed: () async {
              final url = Uri.parse('https://$HOST/api/data/image/${element["problem_string"].toString().substring(2, element["problem_string"].length - 1)}');
              debugPrint("debug : ${element["problem_string"].toString().substring(2, element["problem_string"].length - 1)}");
